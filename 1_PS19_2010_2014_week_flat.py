...
def plot(files):
	...
	colors = ['0.3','0.45','0.6','0.75','0.9']
	fig2= plt.figure()
	ax2 =fig2.add_axes([0.1,0.1,0.8,0.8],polar=True)
	plt.subplots_adjust( left=0.05, bottom=0.05, right=0.95, top=0.95)
	x = np.arange(-180.0,180.0,360/168.)
	theta = (np.pi/180.)*x
	offset =2.
	...
	for count in range(5):
		...
		year = int(f[-8:-4])
		all = reshape(f, year)
		ax2.plot(theta, all, color = colors[count], label = year,linewidth=2)
		# ax2.plot(all)
...
if __name__ == '__main__':
	files = glob.glob('./PS19_2010_2014/MONITOR_'+'*.csv')
	# print files
	plot(files)
